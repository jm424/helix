...
/// Actor-mode single-partition throughput test.
///
/// Same as `test_single_partition_steady_state_throughput` but with actor mode enabled.
/// Use this to compare actor vs non-actor mode performance.
#[tokio::test]
#[ignore] // Run manually with: cargo test -p helix-workload --test steady_state_throughput test_single_partition_actor_mode_throughput -- --ignored --nocapture
async fn test_single_partition_actor_mode_throughput() {
    let profile = load_profile_from_env();
    ...
    let base_port = std::env::var("HELIX_ACTOR_STEADY_BASE_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or_else(|| find_available_base_port(23000, node_count));
    let raft_base_port = std::env::var("HELIX_ACTOR_STEADY_RAFT_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or_else(|| find_available_base_port(33000, node_count));

    let topic = "throughput-actor-mode";
    let topic_name = Arc::new(topic.to_string());

    let report_dir = test_data_dir("actor_mode_throughput_reports");
    std::fs::create_dir_all(&report_dir).expect("failed to create report dir");
    let report_template = report_dir.join("batcher-stats-{node_id}.json");
    std::env::set_var(
        "HELIX_BENCH_REPORT_PATH",
    ...
    let cluster = RealCluster::builder()
        ...
        .raft_base_port(raft_base_port)
        .binary_path(binary_path())
        .data_dir(test_data_dir("actor_mode_throughput"))
        .auto_create_topics(true)
        .default_replication_factor(profile.cluster.replication_factor)
        .topic(topic, profile.cluster.partitions)
        .log_level("warn")
        .actor_mode(true) // Enable actor mode
        .build()
        .expect("failed to start cluster");
    ...
    // Print profile info
    println!("=== Benchmark Profile: {} (ACTOR MODE) ===", profile.name);
    println!("{}", profile.description);
    println!();
    println!("Producer:");
    println!("  threads: {}", profile.producer.threads);
...
/// Actor-mode multi-partition throughput test.
///
/// Same as `test_multi_partition_throughput` but with actor mode enabled.
/// Use this to compare actor vs non-actor mode performance for multi-partition workloads.
#[tokio::test]
#[ignore] // Run manually with: cargo test -p helix-workload --test steady_state_throughput test_multi_partition_actor_mode_throughput -- --ignored --nocapture
async fn test_multi_partition_actor_mode_throughput() {
    let node_count = 3u16;
    let partition_count = 8i32;
    let producer_count = std::env::var("HELIX_MP_PRODUCERS")
        .ok()
    ...
    let base_port = std::env::var("HELIX_MP_ACTOR_THROUGHPUT_BASE_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or_else(|| find_available_base_port(24000, node_count));
    let raft_base_port = std::env::var("HELIX_MP_ACTOR_THROUGHPUT_RAFT_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or_else(|| find_available_base_port(34000, node_count));

    let topic = "throughput-multi-partition-actor";
    ...
    let cluster = RealCluster::builder()
        ...
        .raft_base_port(raft_base_port)
        .binary_path(binary_path())
        .data_dir(test_data_dir("multi_partition_actor_throughput"))
        .auto_create_topics(true)
        .default_replication_factor(3)
        .topic(topic, partition_count as u32)
        .log_level("warn")
